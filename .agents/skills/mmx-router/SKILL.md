---
name: mmx-router
description: SRC 项目的 MiniMax CLI token 路由协议。定义何时必须把数据交给 mmx 处理而非 Codex 直读，以及安全测试专用 prompt 模板。供 stealth-scanner 和 vuln-review 引用。
allowed-tools: Bash
---
...
**Codex 不读原始噪音数据。** 以下数据源必须先交给 mmx 处理，Codex 只读 mmx 返回的精简结果。
