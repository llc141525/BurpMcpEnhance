...
```bash
python3 TOOLS/auth/session_manager.py --target "{目标名}" --role primary
```
...
- 退出码 1 → 输出以下提示后退出：
  ...
  如 login_url 未存储，加参数:
    python3 TOOLS/auth/session_manager.py --target "{目标名}" --role primary --login-url "https://sso.example.com/login?..."
  ```
...
```bash
python3 TOOLS/auth/auth_explore.py --target "{目标名}"
python3 TOOLS/auth/auth_state.py capture --target "{目标名}"
```
...
- PoC / 基线 / 门控请求必须优先携带 DB 中匹配域 Cookie、`X-Id-Token`、`Authorization: Bearer <auth_storage_tokens.token_value>`。
- 每条复核 notes 必须记录 `auth_mode=cookie+bearer|cookie_only|none`；没有 bearer token 的 primary/unauth 结果不得写 confirmed，只能写 low_confidence 或 false_positive。
- `SP-BLH-*` 复核前必须重新执行 token 态 baseline；若 baseline 未携带 bearer token，复核结论不得 confirmed。
- 若 `auth_explore.py` 因 mmx CLI 缺失跳过 hunt_queue 分类，不视为认证失败；继续使用已捕获 token 复核。
...
3. **Session 过期** — 任意验证请求返回 302/401 或跳登录页时，执行 `python3 TOOLS/auth/session_manager.py --target "{目标名}" --role primary`；成功后重试当前请求一次，失败才输出 `[AUTH_BARRIER]` 并退出。
4. **高危** — RCE/SQL 写 shell/任意文件上传，立即升级操作员，不继续。
...
### Step 4c — 变种分析 Phase B: 代码级模式搜索（可选，Codex 驱动）
...
Phase B 非自动——由 Codex 判断是否值得深入。对于大部分 `sqli`/`xss`/`command_injection` 类型，Phase A 的端点搜索已足够。
