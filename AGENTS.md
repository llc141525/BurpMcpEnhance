...
| **Codex（AI）**  | 分析 Burp 流量、构造 PoC、验证漏洞、JS 逆向、编写报告 |
...
**临时文件铁律**：所有临时文件（脚本、JSON、日志、HTML 响应、JS 反混淆产物）必须写到 `tmp/` 目录，**禁止**写入根目录或 `.Codex/`。
...
三个独立 Codex session 通过 SQLite 协同工作，可同时运行：
...
2. 回到 Codex 调用 manual-replay skill
...
4. 如需分析读到的代码片段内容 → 喂 etl_analyzer 提取关键逻辑
