# manual-replay Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build manual-replay workflow skill — operator runs business flows in Burp, AI reads history and generates variant attacks (IDOR/unauth/param_logic/captcha_reuse/user_enum/password_reset).

**Architecture:** Standalone skill sharing hunt_queue table (with source='manual_replay') + auth_sessions with business-logic-hunt. Four phases: collect (time-window Burp capture) → analyze (mmx intent + flow analysis) → variant_gen (intent→template mapping) → execute (Burp replay + comparison).

**Tech Stack:** Burp MCP (history + repeater), mmx CLI (classification), TOOLS/db_query.py, SQLite, existing business-logic-hunt patterns.
...
|------|--------|----------------|
| `migrations/006_add_hunt_queue_source.sql` | Create | Add source + flow_id to hunt_queue |
| `.claude/skills/manual-replay/SKILL.md` | Create | Full four-phase workflow skill |
| `CLAUDE.md` | Modify | Skills table entry + workflow 2.6 |

---

### Task 1: Migration 006 — hunt_queue source/flow_id

**Files:**
- Create: `migrations/006_add_hunt_queue_source.sql`

- [ ] **Step 1: Create migration SQL**

`migrations/006_add_hunt_queue_source.sql`:
```sql
...
```bash
python3 TOOLS/migrate.py --target "test" --init 2>/dev/null; python3 TOOLS/migrate.py --target "test" --status
# If not applied:
python3 TOOLS/migrate.py --target "test"
```

- [ ] **Step 3: Verify columns exist**

```bash
python3 TOOLS/db_query.py --target "test" "PRAGMA table_info(hunt_queue)"
```

Expected output includes `source` (TEXT, default 'auto') and `flow_id` (TEXT) columns.

- [ ] **Step 4: Commit**

```bash
git add migrations/006_add_hunt_queue_source.sql
git commit -m "feat: add source/flow_id columns to hunt_queue for manual-replay"
```

---

### Task 2: SKILL.md — Complete manual-replay workflow

**Files:**
- Create: `.claude/skills/manual-replay/SKILL.md`

- [ ] **Step 1: Write SKILL.md**

`.claude/skills/manual-replay/SKILL.md`:
...
python3 -c "
import re
with open('.claude/skills/manual-replay/SKILL.md') as f:
    content = f.read()
meta = dict(re.findall(r'^(\w+):\s*(.+)$', content.split('---')[1], re.MULTILINE))
assert meta['name'] == 'manual-replay'
assert 'mcp__burp__' in meta['allowed-tools']
...
git commit -m "feat: add manual-replay skill — operator-driven business logic variant attack workflow"
```

---

### Task 3: CLAUDE.md — Skills table + workflow 2.6

**Files:**
- Modify: `CLAUDE.md`

- [ ] **Step 1: Add manual-replay to Skills table**

Read current Skills table area in CLAUDE.md, insert new row after business-logic-hunt:

```markdown
| **manual-replay** | 操作员跑业务流程 → AI 变种攻击。时间窗口 Burp 采集 → mmx 分类 → 流分析 → 变种生成 → 三层执行 | `Skill(skill="manual-replay", args="目标: 台州学院; 模式: replay; 窗口: 5")` |
```

- [ ] **Step 2: Add workflow 2.6 section**

Insert between existing 2.5 (business-logic-hunt) and 3 (vuln-review):
...
```bash
git add CLAUDE.md
git commit -m "docs: add manual-replay to skills table and workflow 2.6"
```
...
```bash
# Check CLAUDE.md references manual-replay
grep -n "manual-replay" CLAUDE.md

# Check migration applied
python3 TOOLS/migrate.py --target "test" --status 2>/dev/null

# Check skill file exists and has correct structure
ls -la .claude/skills/manual-replay/SKILL.md
```

Expected: all references consistent, migration 006 applied, SKILL.md present.

- [ ] **Step 2: Dry-run key SQL queries against test target**

```bash
# Create test target if not exists
python3 TOOLS/db_query.py --target "test" --init

# Verify source column
python3 TOOLS/db_query.py --target "test" "PRAGMA table_info(hunt_queue)"

# Verify flow_id column
python3 TOOLS/db_query.py --target "test" "SELECT sql FROM sqlite_master WHERE type='table' AND name='hunt_queue'"
```

Expected: hunt_queue has source (TEXT, default 'auto') and flow_id (TEXT) columns with indexes.
...
Verify clean state, no unexpected files.
...
|-------------|------|----------|
| 1. Architecture — four-phase model | Task 2 SKILL.md | Full — collect→analyze→variant_gen→execute phases |
| 2. Time-window collection + mmx | Task 2 collect phase | Full — Burp pagination, filtering, mmx classification |
| 3. Variant generation engine | Task 2 variant_gen phase | Full — intent→template mapping, AI generation |
| 4. Execution + output | Task 2 execute phase | Full — Burp send, comparison, findings/suspicious_points |
| 5. DB Schema + external impact | Task 1 migration, Task 3 CLAUDE.md | Full — migration 006, CLAUDE.md update |
| 6. Error handling + escalation | Task 2 SKILL.md | Full — error table + operator escalation |
