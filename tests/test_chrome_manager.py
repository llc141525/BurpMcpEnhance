...
def test_is_chrome_running_returns_true_when_port_responds():
    with patch("requests.get") as mock_get:
        ...
        from TOOLS.auth.chrome_manager import is_chrome_running
...
def test_wait_for_chrome_returns_cdp_url_when_ready():
    with patch("TOOLS.auth.chrome_manager.is_chrome_running", side_effect=[False, False, True]):
        with patch("time.sleep"):
            from TOOLS.auth.chrome_manager import wait_for_chrome
            result = wait_for_chrome(port=9222, timeout=5)
            assert result == "http://localhost:9222"


def test_wait_for_chrome_raises_on_timeout():
    with patch("TOOLS.auth.chrome_manager.is_chrome_running", return_value=False):
        with patch("time.sleep"):
...
def test_write_cdp_url_to_db():
    ...
    from TOOLS.auth.chrome_manager import write_cdp_url_to_db
